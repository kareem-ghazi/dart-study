...
2.   Return the result of invoking the `==` method on _x_ with the argument _y_. (That's right, operators such as `==` are methods that are invoked on their first operand. For details, see [Operators](https://dart.dev/language/methods#operators).)
